...
rooster_tail = Bait.create!(
  ...
  description: "There's a good reason this colorful, spinning bait has been around since the 1950s: it flat out produces. It's also one of guleria's favorites. 😉"
)
...
FishCatch.where(user: guleria).each do |fish_catch|
  Like.create!(user: arun, fish_catch: fish_catch)
end
